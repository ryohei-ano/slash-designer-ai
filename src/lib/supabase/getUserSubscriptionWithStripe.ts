...
const safeSupabase = supabase || {
  from: () => ({
    select: () => ({
      eq: () => ({
        eq: () => Promise.resolve({ data: null, error: new Error("Supabaseクライアントが初期化されていません") })
      })
    })
  })
};
...
export async function getUserSubscriptionWithStripe(
  ...
  try {
    ...
    // 詳細なログ出力を追加
    console.log(`[getUserSubscriptionWithStripe] 検索結果: userId=${clerkUserId}, allSubs=`, allSubscriptions, listError);
    ...
    if (!allSubscriptions || allSubscriptions.length === 0) {
      console.log(`[getUserSubscriptionWithStripe] ユーザー ${clerkUserId} の契約情報が見つかりません`);
      return null;
    }
    ...
    try {
      ...
      const currentPeriodEnd = (stripeSub.items.data[0] as any).current_period_end || 
                              (stripeSub as any).current_period_end || 0;
