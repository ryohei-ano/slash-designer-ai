...
export async function syncWorkspaceWithDify(workspaceId: string) {
  try {
    ...
    if (!textSyncResult.success) {
      ...
      return { success: false, error: `テキストデータの同期に失敗しました: ${textSyncResult.error}` }
    }
    ...
    if (files && files.length > 0) {
      ...
      if (!fileSyncResult.success) {
        ...
        return { success: false, error: `ファイルデータの同期に失敗しました: ${fileSyncResult.error}` }
      }
    }
...
${data.urls.length > 0 ? `関連URL: ${data.urls.join(', ')}` : ''}
    ...
    return { 
      success: false, 
      error: error instanceof Error ? error.message : "テキストデータ同期中に不明なエラーが発生しました" 
    }
  }
}
...
async function syncFileData(files: any[], workspaceId: string) {
  try {
    ...
    return { 
      success: false, 
      error: error instanceof Error ? error.message : "ファイルデータ同期中に不明なエラーが発生しました" 
    }
  }
}
