 ...
 */
export async function saveSubscription(clerkUserId: string, stripeCustomerId: string, priceId: string, subscriptionId: string) {
...
export async function createCustomerPortalSession(): Promise<{ url: string | null; error?: string }> {
  try {
    ...
    console.log(`[createCustomerPortalSession] 検索結果: clerkUserId=${userId}, data=`, data, "error=", error);
...
export async function createCheckoutSession(priceId: string): Promise<{ url: string | null; error?: string }> {
  try {
    ...
    console.log(`[createCheckoutSession] セッション作成成功: sessionId=${session.id}, url=${session.url}`);
    return { url: session.url }
  } catch (error) {
 ...
 */
export async function assignFreeSubscription(userId: string): Promise<{ success: boolean; error?: string }> {
  try {
    ...
    console.log(`[assignFreeSubscription] Stripeサブスクリプション作成成功: subscriptionId=${subscription.id}`)
