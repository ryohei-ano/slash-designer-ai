...
export async function POST(req: Request) {
  try {
    ...
    if (!DIFY_API_KEY || !DIFY_API_URL) {
      return NextResponse.json({ error: 'Dify API設定が見つかりません' }, { status: 500 })
    }
    ...
    if (!difyResponse.ok) {
      ...
      return NextResponse.json({ error: 'Dify APIリクエストに失敗しました' }, { status: 500 })
    }
    ...
    if (!stream) {
      return NextResponse.json({ error: 'ストリームの取得に失敗しました' }, { status: 500 })
    }
